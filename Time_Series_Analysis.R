...
IPG2211A2N.9 <- read.csv("~/Downloads/IPG2211A2N.csv")
head(IPG2211A2N.9)
class(IPG2211A2N.9)
IPG2211A2N.9 <- as.vector(t(as.matrix(IPG2211A2N.9[,2:13])))/1000 
head(IPG2211A2N.9)
IPG2211A2N.9.ts <- ts(IPG2211A2N.9, c(2000,1), end = c(2018,12), frequency = 12) 
str(IPG2211A2N.9.ts)
start(IPG2211A2N.9.ts);end(IPG2211A2N.9.ts); frequency(IPG2211A2N.9.ts)
plot(IPG2211A2N.9.ts, xlab = "DATE", ylab = "IPG2211A2N")

IPG2211A2N.9 <- read.csv("~/Downloads/IPG2211A2N-3.csv")
str(IPG2211A2N.9)
mpls.date <- as.POSIXct(as.character(IPG2211A2N.9$DATE, format="%Y-%m-%d %H:%M:%S"))
IPG2211A2N.9.zoo <- zoo(IPG2211A2N.9$IPG2211A2N, order.by = mpls.date)
str(IPG2211A2N.9.zoo)
is.regular(IPG2211A2N.9.zoo, strict = TRUE);is.regular(IPG2211A2N.9.zoo, strict = FALSE)
sum(is.na(IPG2211A2N.9.zoo))
IPG2211A2N.9.zoo[is.na(IPG2211A2N.9.zoo)==TRUE]
IPG2211A2N.9.zoo2<- na.approx(IPG2211A2N.9.zoo)
class(IPG2211A2N.9.zoo2)
na.filled <- window(IPG2211A2N.9.zoo2, start= as.POSIXct("2000-01-01"), end = as.POSIXct("2019-01-01"))
na.filled
plot.zoo(na.filled, xlab = "DATE", ylab = "IPG2211A2N", col = "mediumblue", lwd = 2) 
title("Electric and Gas Utilities")

test <- read.csv("~/Downloads/IPG2211A2N-3.csv", header = TRUE)
head(test)
class(test)
everglades.ts <- ts(test, c(2000,1), end = c(2019,1), frequency = 12) 
str(everglades.ts)
start(everglades.ts);end(everglades.ts); frequency(everglades.ts)
plot(window(everglades.ts, start = c(2000,1), end = c(2019,1), frequency = 12), ylab = "Visitors (1000's)", xlab = "Time (years)",col = "coral3", lwd = 2)
